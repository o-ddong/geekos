...
/*
    init context(User_Context) and set memorysize from parameter(size)
*/
...
/*
    /inuos2018/src/gekkos/segment.c
    Init_LDT_Descriptor(struct Segment_Descriptor* desc,
...
/*
    ## Init & Set Selector
    /inuos2018/include/geekos/segment.h
    static __inline__ ushort_t Selector(int rpl, bool segmentIsInGDT, int index)
    rpl: Requestor Privilege Level / KERNEL -> 0, USER -> 3 

    /inuos2018/src/geekos/gdt.c
    Get_Descriptor_Index(struct Segment_Descripter *desc)
...
/*
    /inuos2018/src/geekos/segment.c
    Init_(Data, Code)_Segment_Descriptor(struct Segment_Descriptor* desc, 
					ulong_t baseAddr,
					ulong_t numPages,
					int privilegeLevel)
*/
//    TODO("Initialize code and data segments within the LDT");
    ...
    context->csSelector = Selector(USER_PRIVILEGE, false, 0); // code_seg = 0
    context->dsSelector = Selector(USER_PRIVILEGE, false, 1); // data_seg = 1
...
//    TODO("Find maximun virtual address");
    for (int idx = 0; idx < exeFormat->numSegments; idx++) {
	struct Exe_Segment* segment = &exeFormat->segmentList[idx];
...
/*
    ...
    return value at each address -> &numArgs, &argBlockSize
    (inside func)-> argLen get from commnad
*/
...
/*
    Format_Argument_Block(char *argBlock, unsigned numArgs, ulong_t userAddress, const char *commnad)
    MUST CALL FISRT -> Get_Argument_Block_Size (To Get #numArgs)
*/
//    TODO("Format argument block");
    Format_Argument_Block(userContext->memory + argBlockAddr, numArgs, argBlockAddr, command);

// ## Fill & init UserContext
