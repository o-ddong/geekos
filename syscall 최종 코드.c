...
static int Sys_PS(struct Interrupt_State *state) {
	...
	struct Kernel_Thread *process = (&s_allThreadList)->head;
	int i, res;
	bool copy;

	copy =	Copy_From_User(ptable, state->ebx, entries * sizeof(struct Process_Info));
	if (!copy)
		return -7;
	...
	for (i = 0; i < entries; i+=1) {
		...
		if (process->userContext == 0)
			memcpy(ptable[i].name, process->threadName, 20);
		else
			memcpy(ptable[i].name, process->userContext->name, MAX_PROC_NAME_SZB);

		ptable[i].pid = process->pid;

		if (process->owner != 0)
			ptable[i].parent_pid = process->owner->pid;
		else
			ptable[i].parent_pid = 0;

		ptable[i].priority = process->priority;

		if ((process->refCount == 0 && !(process->alive)) || (process->refCount == 1 && !(process->alive)))
			ptable[i].status = STATUS_ZOMBIE;
		if (process->refCount == 1 && process->alive)
			ptable[i].status = STATUS_BLOCKED;
		if (process->refCount == 2 && process->alive)
			ptable[i].status = STATUS_RUNNABLE;

		ptable[i].affinity = process->affinity;
		ptable[i].currCore = 0;
		ptable[i].totalTime = process->totalTime;

		process = process->nextAll_Thread_List;
	}

	copy = Copy_To_User(state->ebx, ptable, entries * sizeof(struct Process_Info));
	if (!copy)
		res = -1;
	else
		res = i;
...
static int Sys_Kill(struct Interrupt_State *state) {
	...
	for (i = 0; i < entries; i+=1) {
		...
		if ((unsigned int) process->pid == state->ebx) {
			if (process->userContext == 0)
				return -1;
			else
				break;
	...
	// Print("process->pid = %d\n", process->pid);
    	process->alive = false;
	// Print("process->alive = %d\n", process->alive);

	iflag = Begin_Int_Atomic();
	// Print("Begin iflag = %d\n", iflag);
	// Alarm_Cancel_For_Thread(process);

	// Tlocal_Exit(process);

	Wake_Up(&process->joinQueue);
	// Print("Wake_UP\n");
